...
The pinned client's redirect/static-header behavior is an implementation release gate: exercise discovery, startup and forwarded requests against cross-host, subdomain and changed-port redirect sinks, as well as poisoned inherited proxies. The sink must receive no protected header. Use supported pinned configuration to disallow redirects where available; verify scope enforcement in released source and binary. If v0.0.14 cannot satisfy this boundary, stop and present the minimal necessary official release upgrade with evidence and new integrity pins for review; do not patch the client, add a custom proxy, or silently relax the requirement. This is an explicit compatibility risk, not claimed existing coverage.
...
- v0.0.14 may need writable files or have unsuitable redirect behavior → bounded release compatibility gate before implementation is called complete; justify any minimal official upgrade and repeat evidence.
