...
Large 64-bit integer Account IDs (e.g., `987654321098765431`) are suffering from precision loss during JSON serialization/deserialization when treated as numbers in MCP tool calls. This results in API errors like "Nonexisting acc_id" because the ID is rounded to a float representation (e.g., `987654321098765000`).
