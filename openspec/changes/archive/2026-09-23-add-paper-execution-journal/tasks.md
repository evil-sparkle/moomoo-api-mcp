...
Development implementation and automated acceptance are complete. Tasks 1.0–1.3 and 1.5–1.6
...
- [x] 2.1 Extend the Stage 1 `Settings` with the journal configuration: the
...
- [x] 2.2 Ensure `READ_ONLY` resolves no journal path and constructs no store. Verify
...
- [x] 2.3 Reject a `SIMULATE` configuration whose journal settings are incomplete,
  naming the variable. Verify that the process exits before any transport is served.
  (`U14`)

## 3. `ExecutionStore`

- [x] 3.1 Add `services/execution_store.py` with short, worker-owned connections:
...
- [x] 3.2 Apply the pragmas from design Decision 3: `journal_mode = DELETE`,
...
- [x] 3.3 Implement the schema, its version in `PRAGMA user_version`, and the
...
- [x] 3.4 Enforce single executor process access using an exclusive non-blocking OS
...
- [x] 3.5 Implement admission epochs: record a fresh epoch on startup, stamp each
  ...
  (`U01`, `U12`, `C04`)
- [x] 3.6 Implement explicit, short transactions with `BEGIN IMMEDIATE` for writes,
...
- [x] 3.7 Implement two-phase admission: persist the token and request in state
  ...
  refusals. (`U01`, `U05`)
- [x] 3.8 Implement the audit log table `recovery_audit` for recording operator
...
- [x] 3.9 Store outstanding operator-review requirements durably and **independently of
...
- [x] 4.1 Add `services/execution_identity.py`: canonicalization over environment,
...
- [x] 4.2 Carry prices as strict decimal strings end to end and compare by decimal
  value. Require a string for the paper price branch of the shared mutation schema, and
  **refuse** a numeric paper price
  ...
  supplied. (`U03`)
- [x] 4.3 Fingerprint a modification over the target order ID and caller's supplied
  ...
  conflict. (`U04`)
- [x] 4.4 Validate the operation identifier's form: non-empty, printable, within a
...
- [x] 4.6 Bind the **concrete resolved account** at admission: resolve `acc_id="0"` to
...
- [x] 4.5 Return an immediate bounded response (`IN_FLIGHT`) when an identical retry
...
- [x] 5.1 Implement the lifecycle in design Decision 7's order: validate token and
...
- [x] 5.2 Implement serialized dispatch execution, with the execution lock acquired
...
- [x] 5.7 Assert that concurrent modifications of one order cannot restore an omitted
  ...
  execution lock. (`U02`)
- [x] 5.3 Refuse mutations when storage cannot be written before dispatch, stating
  that no order was sent, and never dispatch unjournaled. Verify with storage fault
  injection. (`U06`)
- [x] 5.4 Record outcomes onto Stage 1's three-way classification, keeping local
...
- [x] 5.5 Report late local failures with broker evidence separated from the local
  ...
  (`U09`)
- [x] 5.6 Run subprocess crash tests covering all crash windows: between admit and
...
- [x] 6.1 Implement candidate matching versus proof of ownership in reconciliation:
...
- [x] 6.2 Assert that unrelated identical broker orders do not resolve an operation,
...
- [x] 6.3 Record partial fills, complete fills and cancellations as mutually distinct
...
- [x] 6.4 Implement the recovery review gate at startup: enumerate non-terminal
...
- [x] 6.5 Implement the named operator recovery acknowledgement tool
  ...
  that commit. (`U17`)
- [x] 6.6 Support evidence-backed accounting of terminal target orders
...
- [x] 6.7 Ship exactly one operator disposition in version 1, `TERMINAL_ACCOUNTED`.
  ...
  `NOT_SENT`. (`U20`)
- [x] 6.10 Require an authorized operator acknowledgement for **every dispatch marker
  ...
  (`U07`, `U19`)
- [x] 6.11 Accept indefinite blocking: an operation accountable by neither
...
- [x] 6.8 Implement the operator authorization boundary: a capability distinct from the
...
- [x] 6.9 Implement release-by-blocking-reason: an unresolved outcome clears on
...
- [x] 7.1 Enforce the configured simulated-account allowlist in `PaperExecution` alongside
  `TradingPolicy`, requiring
  verification that an allowlisted account is a simulated account, and refusing any
  other account before a gateway mutation (explicitly unallowlisted IDs before discovery). Verify with tests for an un-allowlisted
  account, an unverifiable account, and resolution when exactly one is eligible.
  (`U14`)
- [x] 7.2 Enforce independent paper journal blocking: when `JOURNAL_BLOCKED`, all
  paper mutations fail closed; `lock_trade` has no effect on journal blocking.
  Verify with tests. (`U15`)
- [x] 7.3 Enforce the version 1 paper scope, refusing anything outside it rather than
...
- [x] 7.4 Assert that no tool argument can elevate the configured mode or reach
...
- [x] 8.1 Require `operation_id` and `admission_epoch` on paper mutations in
...
- [x] 8.2 Expose the `acknowledge_recovery` tool behind the operator capability from
...
- [x] 8.3 Report journal state in health without a gateway request, applying the
...
- [x] 8.4 Report unreachable or unwritable journal storage explicitly while keeping
  connectivity probes accurate. Verify with a test making storage unwritable. (`U18`)

## 9. Deployment and documentation

- [x] 9.1 Add the optional `execution-data` volume and its directory, owned by the
...
- [x] 9.2 Confirm a `READ_ONLY` deployment starts and serves with no journal volume
  mounted and no database anywhere. Verify with a container test. (`C01`)
- [x] 9.3 Document consistent backup procedures and supported POSIX locking/fsync
...
- [x] 9.4 Document, in the operator runbook, that reinitializing, replacing or
...
- [x] 10.1 Run the full local gate and confirm each command passes: `uv run ruff check
...
- [x] 10.2 Run container tests for missing mount, single-process locking, consistent
  backup, and restoring older storage, confirming that restore requires recovery
  review. (`C01`–`C04`)
- [x] 10.3 Verify that every scenario in this change's delta specs maps 1:1 to a
...
## 11. Manual verification — tracked separately

Former tasks 11.1 and 11.2 (`M01`–`M04`) moved, uncompleted, to
`validate-paper-execution-provider` tasks 2.1–2.3. Separate authorization remains
required for provider mutations and controlled response loss. Development archive
and automated test results do not establish provider or caller-chain behavior.

## Scenario-to-test traceability

Every scenario in this change's delta specs maps to a planned test. 147 scenarios across 16 requirements.
...
|  | Paper journal survives deployment mode changes | `U01`, `U14` |
